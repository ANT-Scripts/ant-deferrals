...
function playerConnect(source, deferrals)
    ...
    if Config.RequireSteam and not Queue:IsSteamRunning(source) then
        -- prevent joining
    ...
    Queue:CanJoin(source, function(reason)
    ...
    if Queue:IsInConnecting(ids, false, true) then
        ...
        if Queue:NotFull() then
            ...
            if not Queue:IsInQueue(ids) then
                Queue:AddToQueue(ids, connectTime, name, source, deferrals)
            end
    ...
    if Queue:IsInQueue(ids) then
        rejoined = true
        Queue:UpdatePosData(source, ids, deferrals)
